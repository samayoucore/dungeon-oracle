...
export default function NarrativeLog({ onTypingChange }: NarrativeLogProps) {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      ...
      <div ref={scrollRef} onScroll={handleScroll} className="flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto p-4">
        {entries.length === 0 && <p className="text-sm text-muted">Подземелье ждёт твою историю…</p>}
        {entries.map((entry, index) => (
          <div key={entry.id} className="rounded-md border border-white/10 bg-dungeon/30 px-3 py-2">
